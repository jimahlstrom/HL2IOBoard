...
// This firmware uses the Hardrock-500 serial port commands to do band changes
...
// Translate a frequency code to an HR500 band code. Out-of-band codes are mapped to
// 0 (Unknown), not the nearest band. That way the HR500 will refuse to transmit
...
//	 }
	...
	case 1: 	// Start tuning
		// Set ATU active
		uart_puts_log(UART_ID, "HRTB1;");
		// Query ATU Mode
		uart_puts_log(UART_ID, "HRTB;");
		state_antenna_tuner = 4;
		//Registers[REG_ANTENNA_TUNER] = state_antenna_tuner;
		break;
	case 2: 	// Bypass tuner
		// Set ATU to bypass
		uart_puts_log(UART_ID, "HRTB0;");
		// Query ATU Mode
		uart_puts_log(UART_ID, "HRTB;");
	...
	case 3: 	// Check ATU mode
		{
			char *cmd[2] = {"",""};
			uint8_t status = handle_serial_response("HRTB0;", cmd);
	...
	case 4: 	// Check ATU mode, then tune on next TX
		{
			char *cmd[2] = {"HRTU;",""};
			uint8_t status = handle_serial_response("HRTB1;", cmd);
			if (status == RESPONSE_GOOD) {
	...
	case 7: 	// Check if transmit started
		if (absolute_time_diff_us(tuner_time1, get_absolute_time()) / 1000 >= 1000) {
			...
			char *cmd[2] = {"",""};
			uint8_t status = handle_serial_response("HRTT1;", cmd);
#if DEBUG
			printf("status: %d\n", status);
#endif
			if (status == RESPONSE_GOOD) {
	...
	case 10:
		{
			char *cmd[2] = {"",""};
			uint8_t status = handle_serial_response("HRTRS;", cmd);
			if (status == RESPONSE_GOOD) {
				// Tuning succeeded
				//Registers[REG_ANTENNA_TUNER] = 0;
				state_antenna_tuner = 0;
	...
	case 11:
		{
			char *cmd[2] = {"",""};
			uint8_t status = handle_serial_response("HRTRT;", cmd);
			if (status == RESPONSE_GOOD) {
				// Tuning succeeded
				//Registers[REG_ANTENNA_TUNER] = 0;
				state_antenna_tuner = 0;
