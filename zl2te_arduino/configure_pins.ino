...
void configure_pins(bool use_uart1, bool use_pwm4a) {
  ...
  gpio_init(GPIO18_In2);
  gpio_set_dir(GPIO18_In2, GPIO_IN);
  gpio_disable_pulls(GPIO18_In2);
  gpio_init(GPIO21_In3);
  gpio_set_dir(GPIO21_In3, GPIO_IN);
  gpio_disable_pulls(GPIO21_In3);
  gpio_init(GPIO07_In4);
  gpio_set_dir(GPIO07_In4, GPIO_IN);
  gpio_disable_pulls(GPIO07_In4);
  gpio_init(GPIO06_In5);
  gpio_set_dir(GPIO06_In5, GPIO_IN);
  gpio_disable_pulls(GPIO06_In5);
  gpio_init(GPIO13_EXTTR);
  gpio_set_dir(GPIO13_EXTTR, GPIO_IN);
  gpio_disable_pulls(GPIO13_EXTTR);
  gpio_set_irq_enabled_with_callback(GPIO13_EXTTR, 0xc, 1, IrqRxTxChange);  // call on pin change

  // configure output pins
  gpio_init(GPIO25_LED);
  gpio_set_dir(GPIO25_LED, GPIO_OUT);
  gpio_put(GPIO25_LED, 0);
  gpio_init(GPIO00_HPF);
  gpio_set_dir(GPIO00_HPF, GPIO_OUT);
  gpio_put(GPIO00_HPF, 0);
  gpio_init(GPIO01_Sw12);
  gpio_set_dir(GPIO01_Sw12, GPIO_OUT);
  gpio_put(GPIO01_Sw12, 0);
  gpio_init(GPIO02_RF3);
  gpio_set_dir(GPIO02_RF3, GPIO_OUT);
  gpio_put(GPIO02_RF3, 0);
  gpio_init(GPIO03_INTTR);
  gpio_set_dir(GPIO03_INTTR, GPIO_OUT);
  ...
  gpio_init(GPIO12_Sw5);
  gpio_set_dir(GPIO12_Sw5, GPIO_OUT);
